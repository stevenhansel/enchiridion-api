...
use enchiridion_api::{
    ...
    startup::{announcement_scheduler, web},
};
...
async fn main() -> std::io::Result<()> {
    ...
    let application_type: Vec<String> = env::args().collect();
    if application_type.len() < 2 {
        eprintln!("[error] expected application_type to be web or announcement_scheduler");
        process::exit(1);
    }
    let application_type = &application_type[1];
    if application_type != "web" && application_type != "announcement_scheduler" {
        eprintln!("[error] expected application_type to be web or announcement_scheduler");
        process::exit(1);
    }
    ...
    if application_type == "web" {
        ...
        .await?;
    } else if application_type == "announcement_scheduler" {
        let pool = PgPool::connect(config.database_url.expose_secret())
            .await
            .unwrap();
        ...
        announcement_scheduler(announcement_service).await?;
    }
